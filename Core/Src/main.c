...
void updateOLED(float lat, float lon) 
{
  ...
  SSD1306_Clear();
  SSD1306_GotoXY(1, 1);
  SSD1306_Puts(bufferLat, &Font_11x18, SSD1306_COLOR_WHITE);
  SSD1306_GotoXY(1, 30);
  SSD1306_Puts(bufferLon, &Font_11x18, SSD1306_COLOR_WHITE);
...
int main(void)
{
  ...
  osThreadDef(taskGPS, StartGPSTask, osPriorityAboveNormal, 0, 230);
  taskGPSHandle = osThreadCreate(osThread(taskGPS), NULL);

  /* definition and creation of taskMQTT */
  osThreadDef(taskMQTT, StartMQTTTask, osPriorityNormal, 0, 200);
  taskMQTTHandle = osThreadCreate(osThread(taskMQTT), NULL);

  /* definition and creation of taskOLED */
  osThreadDef(taskOLED, StartOLEDTask, osPriorityNormal, 0, 180);
